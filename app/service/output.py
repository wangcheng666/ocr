...
def make_md(file_info: list, mode: str | MakeMode, image_dir: str = "") -> str:
    """生成 Markdown 文本"""
    return vlm_union_make(file_info, mode, image_dir)


def make_content_list(file_info: list, image_dir: str = "") -> list:
    """生成 content_list"""
    return vlm_union_make(file_info, MakeMode.CONTENT_LIST, image_dir)
...
def write_outputs_to_minio(
    *,
    ...
    if f_dump_md and file_info is not None:
        md_content = make_md(file_info, f_make_md_mode, cut_images_dir)
        writer.write_string(f"{stem}.md", md_content)

    if f_dump_content_list and file_info is not None:
        content_list = make_content_list(file_info, cut_images_dir)
        writer.write_string(
...
def build_output_file_list(
    ...
    """
    根据开关参数构造预期的输出文件列表（供 ZIP 打包使用）

    与 write_outputs_to_minio 的开关参数保持一致。
    """
    files = []
    if f_dump_md:
        files.append(f"{stem}.md")
    if f_dump_content_list:
        files.append(f"{stem}_content_list.json")
    if f_dump_middle_json:
        files.append(f"{stem}_middle.json")
    if f_dump_model_output:
        files.append(f"{stem}_model.json")
    if f_dump_full_page_images and page_count > 0:
        files.extend(f"full_page/{i}.jpg" for i in range(page_count))
    return files
